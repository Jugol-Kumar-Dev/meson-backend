...
//        if(BusinessSetting::where('type', 'ssl_mode')->first()->value == 1){
//            define('IS_LOCALHOST', false);
//            define('SSLCZ_TESTMODE', false);
//        }else{
            define('IS_LOCALHOST', true);
            define('SSLCZ_TESTMODE', true);
//        }
    }
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if (!empty($merchant_trans_id) && !empty($merchant_trans_amount)) {
            ...
            $apiDomain = ((SSLCZ_TESTMODE) ? 1 : 0) ? "https://sandbox.sslcommerz.com" : "https://securepay.sslcommerz.com";

            $requested_url = ($apiDomain . $this->config['apiUrl']['order_validate'] . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $this->setApiUrl($apiDomain . $this->config['apiUrl']['make_payment']);
